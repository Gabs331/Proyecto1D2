#include "UART.h"
#include <stdlib.h>
#define BUFFER_SIZE 4  // Tamaño del buffer (3 digitos + terminador nulo)
char rxBuffer[BUFFER_SIZE];  // Arreglo para almacenar los caracteres
uint8_t rxIndex = 0;  // indice para almacenar caracteres en el array
...
void UART_Num(void){
	...
	if (receivedChar == '\r' || receivedChar == '\n') {  // Si se presiona ENTER
		...
		rxIndex = 0;  // Reiniciar el indice para la siguiente recepcion
...
void uint16_to_string(uint16_t num) {
	char buffer[6];  // Maximo 5 digitos + '\0'
	uint8_t i = 0;
	char temp[6];  // Almacena los digitos en orden inverso
	uint8_t j = 0;
	...
	UART_Write(buffer); // Envia el string por UART
}